...
const Userprofile = () => {
  ...
  const licenseImages = [
    ...
    userData?.license_back ? { uri: userData.license_back } : null,
  ].filter(Boolean);
    ...
    <>
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.mainContent}>
          ...
          <View style={[styles.mainprofilecontainer, { backgroundColor: colors.card }]}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <View style={styles.columnprofile}>
                <Text style={{ fontFamily: 'Roboto-Bold', fontSize: 18, color: colors.text }}>
                  {userData?.full_name || 'Unknown Name'}
                </Text>
                <Text style={{ fontFamily: 'Roboto-Regular', fontSize: 13, color: colors.subText }}>
          ...
          <View style={[styles.infoprofilecontainer, { backgroundColor: colors.card }]}>
            <View style={styles.sublocationcontainer}>
              <Image source={vehicle} style={styles.geopickicon} />
              <Text style={[styles.sublocationtext, { color: colors.text }]}>{userData?.phone_number || 'No Phone'}</Text>
            </View>
            <View style={[styles.separator1, { backgroundColor: colors.border }]} />

            <View style={styles.sublocationcontainer}>
              <Image source={vehicle} style={styles.geopickicon} />
              <Text style={[styles.sublocationtext, { color: colors.text }]}>{userData?.email_address || 'No Email'} </Text>
            </View>
            <View style={[styles.separator1, { backgroundColor: colors.border }]} />

            <View style={styles.sublocationcontainer}>
              <Image source={geopick} style={styles.geopickicon} />
              <Text style={[styles.sublocationtext, { color: colors.text }]}>{userData?.address || 'No Address Provided'}</Text>
            </View>
            <View style={[styles.separator1, { backgroundColor: colors.border }]} />
          </View>
          ...
          {isCourier ? (
            <>
              <View style={[styles.licenseprofilecontainer, { backgroundColor: colors.card }]}>
                <View style={styles.sublocationcontainer}>
                  <Image source={license} style={styles.licenseicon} />
                  {/* Updated to use Image Viewer Logic */}
                  <Pressable onPress={openLicenseViewer}>
                    <Text style={[styles.sublocationtext1, { color: colors.text }]}>View Driver License </Text>
                  </Pressable>
                </View>
              </View>

              <View style={[styles.vehicleprofilecontainer, { backgroundColor: colors.card }]}>
                <View style={{ flexDirection: 'column', }}>
                  <View style={styles.sublocationcontainer}>
                    <Image source={vehicle} style={styles.geopickicon} />
                    <Text style={[styles.sublocationtext, { color: colors.text }]}>
                  ...
                  <View style={styles.sublocationcontainer}>
                    <Image source={vehicle} style={styles.geopickicon} />
                    <Text style={[styles.sublocationtext, { color: colors.text }]}>{userData?.vehicle_brand || 'N/A'}</Text>
                  </View>
                </View>
                <View style={[styles.separator, { backgroundColor: colors.border }]} />
                <View style={styles.sublocationcontainer}>
                  <Image source={theme} style={styles.geopickicon} />
                  <Text style={[styles.sublocationtext, { color: colors.text }]}>{userData?.vehicle_color || 'N/A'}</Text>
                </View>
                <View style={[styles.separator, { backgroundColor: colors.border }]} />
                <View style={styles.sublocationcontainer}>
                  <Image source={platenum} style={styles.geopickicon} />
                  <Text style={[styles.sublocationtext, { color: colors.text }]}>{userData?.plate_number || 'N/A'}</Text>
                </View>
                <View style={[styles.separator, { backgroundColor: colors.border }]} />
                <View style={styles.sublocationcontainer2}>
                  <Image source={feature} style={styles.geopickicon} />
                  <Text style={[styles.sublocationtext2, { color: colors.text }]} >
            ...
            <View style={[styles.licenseprofilecontainer, { marginTop: 20, backgroundColor: colors.card }]}>
              <Text style={{ color: colors.text, textAlign: 'center' }}>User is not registered as a Courier.</Text>
            </View>
          )}
          ...
          <View style={styles.centeredView}>
            <View style={[styles.modalView1, { backgroundColor: colors.card }]}>
              <View style={{ flexDirection: 'row', marginBottom: 16 }}>
                ...
                <Text style={{ fontSize: 18, color: colors.text, fontWeight: 'bold', alignItems: 'center', }}>Are you sure you want to suspend this account? The user will not be able to log in or use the services until reactivated.</Text>
              </View>
              <View style={{ marginBottom: 1 }}>
                <View style={styles.filterbtn1}>
                  <DropDownPicker
                    ...
                    style={[styles.dropdown1, { backgroundColor: colors.background, borderColor: colors.border }]}
                    textStyle={styles.dropdownText1}
                    placeholderStyle={styles.placeholderText1}
                    dropDownContainerStyle={[styles.dropdownContainer1, { backgroundColor: colors.background, borderColor: colors.border }]}
                    selectedItemContainerStyle={[styles.selectedItemContainer1, { backgroundColor: colors.card }]}
              ...
              <View style={{ flexDirection: 'row', gap: 40, marginTop: -140 }}>
                <Pressable
                  style={{ alignSelf: 'center', borderColor: colors.text, borderWidth: 1, borderRadius: 8, paddingVertical: 8, paddingHorizontal: 24, marginTop: 8 }}
